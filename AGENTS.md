...
**Always English**, regardless of the language of the code, content, or conversation.

**Summarize, don't enumerate.** State what the change accomplishes, not a walkthrough of every file
and decision. A subject line is usually enough; add a body only for context the diff can't convey
(why it was needed, a non-obvious constraint, a breaking change).

```
BAD:  fix build su node 22: sitemap/rss e sharp
      <three paragraphs restating each edit>
GOOD: fix build on node 22
```
